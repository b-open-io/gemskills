#!/usr/bin/env python3
"""Install, check, update, or uninstall Lisa's generated Codex adapter."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if [a.get("generated_file") for a in manifest.get("agents", [])] != [AGENT_FILE]:
        print("error: invalid or missing Gemskills agent manifest")
        return 1
    ...
    ownership = load_json(ownership_path, {"manager": "gemskills", "agents": {}})
    record = ownership.get("agents", {}).get(AGENT_FILE)

    if args.uninstall:
        if not record:
            print(f"unchanged: {AGENT_FILE} is not managed by Gemskills")
            return 0
        if (
            destination.is_file()
    ...
    print("Start a new Codex session before invoking gemskills_content.")
    return 0
